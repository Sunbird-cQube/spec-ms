...
export class ScheduleService {
    ...
    async scheduleProcessorGroup(scheduleData: scheduleDto) {
        ...
        if (isValidSchema.errors) {
            ...
            if (result.errorMessage) {
                ...
                let result;
                if(scheduleData?.processor_group_name === 'ingest_data' && scheduleData?.program_name){
                     result = await this.CreateSchedule(scheduleData?.processor_group_name, scheduleData.scheduled_at, scheduleData?.program_name);

                }else{
                    if(scheduleData?.processor_group_name !== 'ingest_data'){
                        result = await this.CreateSchedule(scheduleData?.processor_group_name, scheduleData.scheduled_at);

                    }else{
                        return {code:400, error:"program_name is missing for the processor group ingest_data"}
                    }
                }
    ...
    async CreateSchedule(processorGroupName, scheduledAt, program_name?:string) {
        ...
        for (let pg of pg_group) {
            if (pg.component.name == processorGroupName) {
                ...
                return await this.updateProcessorProperty(pg_source['component']['id'], processorGroupName, scheduledAt, program_name)
            }
        }
        return {
    ...
    async updateProcessorProperty(pg_source_id, processorGroupName, schedulePeriod, program_name) {
